...
void main() => runApp(const ProxyChatDemoApp());

class ProxyChatDemoApp extends StatelessWidget {
  const ProxyChatDemoApp({super.key});

  static final coordinator = ProxyDemoCoordinator();
  ...
  Widget build(BuildContext context) {
    return CupertinoApp.router(
      ...
      restorationScopeId: 'proxy_chat_demo',
...
// Host routes such as [OpenChatRoute] implement [ProxyRoute] and forward
// navigation to [ChatShellRoute.navigate] on the embedded shell.

class ProxyDemoCoordinator extends Coordinator<HostRoute> {
  ProxyDemoCoordinator() {
  ...
  late final chatProxyPath = ProxyPath<HostRoute>.createWith(
    ...
    onAction: (action) async {
      ...
      throw UnimplementedError(
        'Chat routes handle ${action.runtimeType} through ProxyRoute.',
      );
    },
  )..bindLayout(ChatLayout.new);

  /// Host root + proxy path + embedded shell slot paths so
  ...
  /// reflects `/chat` and `/chat/thread/:id`. Only consulted while the proxy is
  /// the active path.
  List<HostRoute> _chatProxyStack() {
...
class ThreadListRoute extends HostRoute {
  ...
  @override
  Widget build(ProxyDemoCoordinator coordinator, BuildContext context) {
          ...
          builder: (context, _) {
            return CustomScrollView(
              slivers: [
                ...
                SliverList.builder(
                  itemCount: ProxyDemoCoordinator.threadSummaries.length,
                  itemBuilder: (context, index) {
                    final summary = ProxyDemoCoordinator.threadSummaries[index];
...
// ============================================================================
// Chat layout + proxy routes
// ============================================================================

/// The layout that renders the [ChatShell]. It resolves to the root
/// coordinator's [ProxyDemoCoordinator.chatProxyPath].
class ChatLayout extends HostRoute with RouteLayout<HostRoute>, RouteGuard {
  ...
  @override
  ProxyPath<HostRoute> resolvePath(ProxyDemoCoordinator coordinator) =>
  ...
  @override
  FutureOr<bool> popGuardWith(ProxyDemoCoordinator coordinator) {
...
abstract class ChatProxyRoute extends HostRoute with ProxyRoute<HostRoute> {
  ChatAppRoute toChatRoute();

  DemoChatShellCoordinator shell(ProxyPath<HostRoute> path) =>
      (path.coordinator as ProxyDemoCoordinator).chatCoordinator;
...
class OpenChatRoute extends ChatProxyRoute {
  @override
...
class OpenThreadRoute extends ChatProxyRoute {
...
void openThread(String messageId) {
  ProxyChatDemoApp.coordinator.push(OpenThreadRoute(id: messageId));
}
          ...
          child: Padding(
            ...
            child: Row(
              children: [
                _barIconButton(
                  icon: CupertinoIcons.back,
                  // Host tryPop closes the thread via ProxyRoute.pop → closeThread().
                  onPressed: () => ProxyChatDemoApp.coordinator.tryPop(),
                ),
